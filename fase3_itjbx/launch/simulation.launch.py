#!/usr/bin/env python3
"""Launch de SIMULACAO da missao fase3_itjbx (itajuba_2026)."""
...
def generate_launch_description():
    params = os.path.join(get_package_share_directory('fase3_itjbx'), 'config', 'simulation.yaml')

    stamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    bag_dir = os.path.expanduser(f'~/evtol/mission_logs/fase3_itjbx_{stamp}')
    ...
    mission = Node(
        package='fase3_itjbx', executable='fase3_itjbx',
        parameters=[params], output='screen')
