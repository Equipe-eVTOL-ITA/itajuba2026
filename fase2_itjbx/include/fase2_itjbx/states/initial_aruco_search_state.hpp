...
/**
 * Portado de sae2026/mission_1 (InitialArucoSearchState) — sobe em linha
 * reta ate z_max_search enquanto verifica se o ArUco ja esta visivel; se
 * confirmar a deteccao antes de chegar la, transita direto pra GO_TO_ARUCO
 * sem precisar do SEARCH_ARUCO retangular.
 */
...
public:
    ...
    std::string act(fsm::Blackboard &blackboard) override {
        ...
        // Move slowly upwards. If it reaches the target altitude (within tolerance), start rectangular search.
        if (move_local_constant_step(drone_, target_pos, 0.5f, 0.3f)) {
            move_local_by_vel_as_position(drone_, 0.0f, 0.0f, 0.0f); // hold position
            drone_->log("Reached max search altitude. Switching to rectangular search.");
